...
export function initializeSimulationsSync() {
  ...
  onSnapshot(collection(db, "simulations"), (snapshot) => {
    const fetchedSims = snapshot.docs.map(doc => {
       ...
       if (!data.thumbnail || data.thumbnail.includes('unsplash.com') || data.thumbnail.startsWith('blob:')) {
           data.thumbnail = NO_LOGO;
       }
       if (!data.heroImage || data.heroImage.includes('unsplash.com') || data.heroImage.startsWith('blob:')) {
           data.heroImage = NO_LOGO;
       }
       if (Array.isArray(data.screenshots)) {
           data.screenshots = data.screenshots.filter((url) => typeof url === 'string' && !url.startsWith('blob:'));
       }
       return data;
    });
